...
switch (_filter) do
{
	...
	case 0:
	{
		...
		_ret pushBack ["U_Rangemaster","Cop Uniforma",0];
		
		if(__GETC__(life_coplevel) >= 3) then
		{
			_ret pushBack ["U_B_CombatUniform_mcam_vest","Komisar",0];
			
		};
		if(__GETC__(life_coplevel) >= 5) then
		{
			_ret pushBack ["U_B_SpecopsUniform_sgg","Hlavny Komisar",0];
		};

		if(__GETC__(life_coplevel) == 7) then
		{
			_ret pushBack ["U_B_CombatUniform_mcam","Cop Sef",0];
